...
namespace hgl::math
{
    ...
    {
        return Color3f(hdr / (Color3f(1.0f) + hdr));
    }
    ...
    {
        // 通过转换为 glm::vec3 使用 GLM 的 clamp，再转换回 Color3f
        return Color3f(glm::clamp(static_cast<glm::vec3>(color), glm::vec3(0.0f), glm::vec3(1.0f)));
    }
    ...
    {
        // 通过转换为 glm::vec4 使用 GLM 的 clamp，再转换回 Color4f
        return Color4f(glm::clamp(static_cast<glm::vec4>(color), glm::vec4(0.0f), glm::vec4(1.0f)));
    }
